    ...
    <div className=" p-10 mb-8 mt-10">
      <div className="flex flex-col md:flex-row justify-between p-2">
        <div className="w-full md:w-2/3 mb-8 md:mb-0">
          ...
          <ul>
            {t("formLinksAttendance", { returnObjects: true }).map((form, index) => (
              <li
                key={index}
          ...
          <div className="mt-4 flex justify-between ">
            <span className="text-sm text-gray-600 mt-2 dark:text-white">
              {t("showingResults", { start: 1, end: 5, total: 20 })}
            </span>
        ...
        <div className="w-full md:w-1/3 md:ml-4">
          <h2 className="text-2xl font-bold text-blue-800 mb-4">
            {t("formQuickLinksAttendance")}
          </h2>
          <ul>
            {quickLinks.map((link, index) => (
              <li key={index} className="mb-4">
                <a
                  href={t("formLinkAttendance")}
                  className=" hover:ml-4 text-lg hover:text-blue-800 transition-all duration-300 ease-in-out cursor-pointer"
                >
                  {t(`quickLinkAttendance`)}
                </a>
              </li>
            ))}
          </ul>
